...
```{r}
#this is the way to do a matrix
...
c(X) #vectorizes / flattens a matrix (sometimes useful) ##this will make the matrix a vector
```
...
inverse_quantile_obj = ecdf(X[, "salary"]) #the "empirical" CDF ##we can ask based on the salary that you want 
##which quantile this will be
...
```{r}
X$has_past_unpaid_loan = factor(X$has_past_unpaid_loan, labels = c("Never", ">=1"))## you cast it to make it easy to read
...
```{r}
MASS::as.fractions(0.99)##go to the mass library and get the as.fract
...
```{r}
install_github("yihui/knitr")## to intal the library from github
```
...
```{r}
data(package = .packages(all.available = TRUE))## this shows you all the data that this has
```