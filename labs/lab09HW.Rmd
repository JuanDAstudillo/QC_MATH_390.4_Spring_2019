...
storms %>%
  group_by(name) %>%
  summarise(max_category = max(as.numeric(as.character(category))), max_wind_speed = max(wind), max_pressure = max(pressure), max_ts_diameter = max(ts_diameter, na.rm = TRUE), max_hu_diameter = max(hu_diameter, na.rm = TRUE))
...
storms %>%
  group_by(category) %>%
  summarise(avg_wind_speed = mean(wind), avg_pressure = mean(pressure), avg_ts_diameter = mean(ts_diameter, na.rm = TRUE), avg_hu_diameter = mean(hu_diameter, na.rm = TRUE))
