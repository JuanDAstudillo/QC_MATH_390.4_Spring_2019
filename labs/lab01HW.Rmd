...
//it is not, but this answer is an aproximatio. Since n of 1/2^n is very large the answer will aproximate
to infinity.
```{r}
#TO-DO
y=(1/2)^(0:499)
...
```{r}
#TO-DO## remember we dont create vector because is 0 and 1. 50 - 50 probability wHich is standard bernoulli.
# create the vector is for setting probabilities
sum(sample(0:1, 100, replace= T ))/100
...
sum(sample(0:1, 500, replace= T, c(0.1,0.9)))/500
...
#sample(strsplit(lorem, "[.] ")[[1]])#[1:10])
paste(paste(sample(strsplit(lorem, "[.] ")[[1]]), collapse = ". "),"." ,sep = "")
...
y = sample(rep(levels, 25), replace = F) ##here we dont need to replace
x_2 = factor(y,levels=levels,ordered = T)
class(x_2)
#x =  sample(rep(levels,25)) ##replace = F by default
##x_2 = factor( x ,levels=levels,ordered= TRUE)## check levels
...
```{r}
#TO-DO
tmp=as.numeric(x_2!="none")##all 0s = no crime =none
...
x_7 = c(x_4,x_5,x_6)
x_8 = matrix(x_7,100,3)## matrix(x, n , p) n = 100, p = 3
x_8
...
```{r}
#TO-DO
temp = matrix(c(rnorm(100, 17, 38), runif(100, -10, 10), rpois(100, 6), rexp(100,9) , rbinom(100, 20, 0.12), sample(rep(c(0,1), c(76,24)))),100,6)
temp
```
